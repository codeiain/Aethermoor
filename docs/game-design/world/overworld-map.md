...
Level 5–10:  Thornwood Forest → Crystal Caves Approach
...
| `millhaven-fields`      | Millhaven Fields   | Forest/Grassland | 1–4         | `millhaven-town`, `thornwood-edge`, `aethermoor-road-south`                                                                             | 50                      | Forage nodes, Goblin Scouts (Level 1–3), Hidden cellar (secret)                                                                                                     | No         |
...
| `aethermoor-city`       | City of Aethermoor | Town (Hub)       | 1–20        | `aethermoor-road-south`, `aethermoor-undercity`, `thornwood-edge`, `goblin-wastes-border`, `crystal-caves-approach`, `aether-road-east` | 300 (persistent shared) | Inn, Blacksmith, Auction House, Guild Hall, Bank, All Class Trainers, Crafting Workshop, Faction Envoys, Housing Registrar, Lore Keeper, Fast Travel Waypoint       | No         |
...
### Region 2 — Thornwood Forest (Level 5–12)

| Zone ID                | Zone Name            | Biome           | Level Range | Connections                                                                              | Max Players/Shard | Key POIs                                                                                                                                                  | Contested?                              |
| ---------------------- | -------------------- | --------------- | ----------- | ---------------------------------------------------------------------------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| `thornwood-edge`       | Thornwood Edge       | Forest          | 5–8         | `aethermoor-city`, `millhaven-fields`, `thornwood-heart`, `goblin-wastes-border`         | 50                | Verdant Pact Envoy Camp, Herb nodes, Ancient stone circle (shrine), Lumber camp (resource)                                                                | No                                      |
| `thornwood-heart`      | Thornwood Heart      | Forest (Dense)  | 7–11        | `thornwood-edge`, `thornwood-crossroads`, `bramble-maze-entrance`, `elder-root-approach` | 50                | Dungeon Entrance: Bramble Maze, Hidden waterfall (secret — lore scroll), Treant elder NPC (quest), Forage: rare herbs                                     | No                                      |
| `thornwood-crossroads` | Thornwood Crossroads | Forest/Clearing | 8–12        | `thornwood-heart`, `goblin-wastes-border`, `ember-plains-north`                          | 100               | **PvP Zone (Verdant Pact vs Aetheric Order)** — Lumber Mill control point, Herb Garden control point, Resource node: Thornwood Lumber (rare crafting mat) | **Yes — Verdant Pact / Aetheric Order** |
| `elder-root-approach`  | Elder Root Approach  | Ancient Forest  | 9–12        | `thornwood-heart`, `elder-root-vault-entrance`                                           | 50                | Dungeon Entrance: Elder Root Vault, Ancient Pact shrine (buff: +10% XP in zone for 1hr), Hidden underground passage (requires Tide Rune)                  | No                                      |
...
| Zone ID                  | Zone Name            | Biome                     | Level Range | Connections                                                                                                 | Max Players/Shard | Key POIs                                                                                                                                                                                        | Contested? |
| ------------------------ | -------------------- | ------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `goblin-wastes-border`   | Goblin Wastes Border | Badlands                  | 7–10        | `aethermoor-city`, `thornwood-edge`, `thornwood-crossroads`, `goblin-wastes-deep`, `crystal-caves-approach` | 50                | Goblin raider camps (Level 7–9), Resource: Iron Ore veins, Abandoned watchtower (scout point)                                                                                                   | No         |
...
| Zone ID              | Zone Name          | Biome              | Level Range | Connections                                                                                     | Max Players/Shard | Key POIs                                                                                                                                                                                                         | Contested?                              |
| -------------------- | ------------------ | ------------------ | ----------- | ----------------------------------------------------------------------------------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| `ember-plains-north` | Ember Plains North | Plains/Volcanic    | 11–14       | `goblin-wastes-deep`, `thornwood-crossroads`, `ember-plains-south`, `emberton-town`             | 50                | Resource: Emberstone, Ember Tribe patrol camps, Lava fissure mini-hazards, Route to Sunken Harbor                                                                                                                | No                                      |
...
═══════════════════════════════════════════════════════════════════
    ...
    └─────────────┬──────────────────────────────┬─────────────┘
                  │                              │
    ┌─────────────┴──────────────────────────────┴─────────────┐
    │            THORNWOOD FOREST LV 5–12                       │
    │  thornwood-edge ─── thornwood-heart ─── elder-root-approach│
    │  [Bramble Maze🏰]        thornwood-crossroads ⚔️          │
    │  [Elder Root Vault🏰]                                     │
    └─────────────────────────────┬────────────────────────────┘
                                  │
    ┌─────────────────────────────┴────────────────────────────┐
...
| 3  | The Bramble Maze              | Thornwood Heart     | `thornwood-heart`         | 5–8         | The Thornmother                     | Medium (25–35 min)     | Uncommon/Rare                   | Living vine mechanic                                    |
| 4  | The Elder Root Vault          | Elder Root Approach | `elder-root-approach`     | 8–12        | Root Warden Kaeseth                 | Long (35–50 min)       | Rare/Epic                       | Light beam puzzle boss                                  |
...
| Field          | Value                                                                                                                                                                                                               |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Zone ID        | (within `thornwood-edge` — town sits at the zone border)                                                                                                                                                            |
| Zone ID (town) | `thornhaven-village` *(sub-zone of thornwood-edge)*                                                                                                                                                                 |
| Region         | Thornwood Forest                                                                                                                                                                                                    |
| Biome          | Forest Village                                                                                                                                                                                                      |
| Level Range    | 5–12                                                                                                                                                                                                                |
| Faction        | Verdant Pact (stronghold)                                                                                                                                                                                           |
| Fast Travel    | No (encourages exploration)                                                                                                                                                                                         |
| **Key NPCs**   | Elder Sylvan Kor (Verdant Pact quest chain), Herbalist Menna (rare herb vendor), Ranger Captain Dusk (PvP faction quests), Woodcrafter Aldis (crafting recipes: Thornwood set), Treant Watcher NPC (Dungeon 3 lore) |
...
| **Lore**       | Deep in the forest, Thornhaven is the Verdant Pact's ancestral home. The treants have protected it for centuries. Dungeon 3 and 4 story hooks originate here.                                                       |
