...
Inlined from shared/aethermoor_shared/auth.py — Docker build context for each
service only includes the service directory, so shared code is vendored here
until the build pipeline supports multi-directory contexts.

See: services/auth/README.md — Architecture section.
"""
import hashlib
import hmac
import os
...
def verify_service_token_value(header_value: str | None) -> None:
    ...
    tolerate clock skew between containers.
    """
    if not header_value:
